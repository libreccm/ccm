...
/**
 * <p> The CMS Dispatcher serves all request made within a content section.
 * This dispatcher is called by the Subsite dispatcher.</p>
 *
 ...
 *   notices that CMS is mounted at <tt>/cms</tt> and hands the request to
 *   the CMS dispatcher.</p></li>
 *
 *   <li><p>The CMS dispatcher determines whether a <tt>Page</tt> has been
 *   registered to the URL <tt>/cheese</tt> in this section via its
 *   {@link com.arsdigita.cms.dispatcher.PageResolver}.</p></li>
 *
 *   <li><p>Since no page is registered to the URL, the CMS dispatcher asks
 *   the content section (via its
 *   {@link com.arsdigita.cms.dispatcher.ItemResolver})
 *   for a content item for <tt>/cheese</tt> in this content section. The
 *   result of this process is a {@link com.arsdigita.cms.ContentItem}
 *   object.</p></li>
 *
 *   <li><p>The CMS dispatcher asks the content section for a <tt>Page</tt>
 *   to use as the "master template" for this item. The content section may
 *   apply item-, type-, or request-specific rules to make this decision
 *   (for example, check a user preference for normal or accessible style,
 *   or a query parameter for a printable version).</p></li>
 *
 *   <li><p>The CMS dispatcher hands the master <tt>Page</tt> object to
 *   the {@link com.arsdigita.sitenode.SiteNodePresentationManager}
 *   to serve the page.</p></li>
 *
 *   <li><p>The presentation manager asks the master <tt>Page</tt> object
 *   for an XML document representing the data for the page.</p></li>
 *
 *   <li><p>The master template begins walking through its component
 *   hierarchy, converting each component to XML by calling its
 *   <tt>generateXML</tt> method. The component responsible for rendering
 *   the content item uses an
 *   {@link com.arsdigita.cms.dispatcher.XMLGenerator} to convert the
 *   content item to XML.</p></li>
 *
 *   <li><p>The presentation manager receives the completed XML document, and
 *   selects an XSL transformer to use for generating the HTML. The
 *   stylesheet on which the transformer is based contains templates
 *   for all styles and all content types in the content section, in
 *   particular those from the file <tt>cms-item.xsl</tt>.</p></li>
 * </ol>
 *
...
public class CMSDispatcher implements Dispatcher, ChainedDispatcher {
     ...
     * 1) fetches the current content section
     * 2) fetches the resource mapped to the current section/URL
     * 3) if no resource, fetches the item associated with the current
     *      section/URL
     * 4) if no item, passes request to the JSP dispatcher, which serves
     *      JSP's, HTML pages, and media from the cms/packages/www directory
     *
    ...
    public void dispatch(HttpServletRequest request,
        ...
        if (s_log.isDebugEnabled()) {
            s_log.debug("Dispatching request for " +
                        new URL(request).toDebugString());
        }

        DeveloperSupport.startStage("CMSDispatcher.dispatch: get resource");
        ...
        if ( remainingUrl.endsWith("/") ) {
            ...
            remainingUrl = remainingUrl.substring
                (0, remainingUrl.length() - ItemDispatcher.FILE_SUFFIX.length());
        ...
        try {
            ...
            if (siteNodeAssetURL != null) {
                s_log.debug("Site node asset found at '" +
                            siteNodeAssetURL + "'");
        ...
        if (resource != null) {
            ...
            if (sm.canAccess(request, SecurityManager.PUBLIC_PAGES)) {
                if (preview) {
                    ...
                    item = getContentItem(section, remainingUrl, ContentItem.LIVE);
                }
                if (item != null) {
            ...
            DeveloperSupport.endStage("CMSDispatcher.dispatch: item dispatch");
        }
        ...
        if (m_adminPagesOnly) {
            ...
            if (url.endsWith(ItemDispatcher.FILE_SUFFIX)) {
                url = url.substring(0, url.length() -
                                    ItemDispatcher.FILE_SUFFIX.length());
