...
public class KEAFilter extends Filter implements OptionHandler {
    ...
    private static final Logger s_log = Logger.getLogger(KEAFilter.class);
	...
	private Stemmer m_Stemmer = null;
	...
	public void buildGlobalDictionaries() throws Exception {
		s_log.debug("--- Building global dictionaries");
	...
	private void buildClassifier() throws Exception {
		...
		s_log.debug("--- Converting instances for classifier");
	    ...
	    s_log.debug("--- Building classifier");
	...
	private double[] featVals(String id, FastVector phraseInfo, 
		...
		if (m_LENGTHfeature) {
			...
			if (original == null) {
				s_log.debug("Missing term id " + id + "... may be a stop word");
	...
	private void convertPendingInstances() throws Exception {
		...
		s_log.debug("--- Converting pending instances");
	...
	throws Exception {
		...
		s_log.debug("-- Converting instance");
...
//			This is often the case with Mesh Terms,
			...
			if (orig.length() > 0) {
				...
				if (id != null) {
					...
					if (!hash.containsKey(id)) {
						...
						hash.put(id, c);
						if (forEval && m_Debug) {
							s_log.warn("Skipping the phrase " + orig + ", which appears twice in the author-assigned keyphrase set.");
						}
					}
				} 
			}
	...
	public static void main(String [] argv) {
		...
		try {
			...
			s_log.warn("KEAFilter failed", ex);
		}
	}
}
