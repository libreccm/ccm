...
public class KEAKeyphraseExtractor implements OptionHandler {
    ...
    private static final Logger s_log = Logger.getLogger(KEAKeyphraseExtractor.class);
	...
	public void extractKeyphrases(Hashtable stems) throws Exception {
		...
		s_log.debug("-- Extracting Keyphrases... ");
		// Extract keyphrases
		...
		while (elem.hasMoreElements()) {	
			...
			try {
				...
				if (m_debug) {
					s_log.debug("Can't read document " + str + ".txt", e);
				}
			...
			try {
				...
				if (m_debug) {
				    s_log.debug("No existing keyphrases for stem " + str + ".", e);
				}
			...
			s_log.debug("-- Keyphrases and feature values:");
			...
			for (int i = 0; i < m_numPhrases; i++) {
				if (topRankedInstances[i] != null) {
					...
					if (s_log.isDebugEnabled()) {
						s_log.debug(topRankedInstances[i]);
					}
				}
			}
			if (numExtracted > 0) {
				if (s_log.isDebugEnabled()) {
					s_log.debug("-- " + numCorrect + " correct");
				}
		...
		s_log.debug("Avg. number of matching keyphrases compared to existing ones : " +
				Utils.doubleToString(avg, 2) + " +/- " + 
				Utils.doubleToString(stdDev, 2));
		s_log.debug("Based on " + stats.size() + " documents");
		// m_KEAFilter.batchFinished();
	}
	
	private void buildGlobalDictionaries (Hashtable stems) throws Exception  {
		
		s_log.debug("--- Building global dictionaries from the test collection.. ");
	...
	public void loadModel() throws Exception {
		...
		if (m_buildGlobal == true) {
			s_log.debug("-- The global dictionaries will be built from this test collection..");
	...
	public static void main(String[] ops) {
		...
		try {
			...
			kmb.setOptions(ops);      
			s_log.debug("Extracting keyphrases with options: ");
			...
			for (int i = 0; i < optionSettings.length; i++) {
			    s_log.debug(optionSettings[i] + " ");
			}
			s_log.debug("");
			
			// Loading selected Model:
			s_log.debug("-- Loading the Model... ");
			kmb.loadModel();
			...
			while (en.hasMoreElements()) {
				...
				s_log.debug(option.synopsis());
				s_log.debug(option.description());
			}
		}
	}
}
