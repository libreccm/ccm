...
public class Vocabulary implements Serializable {
    ...
    private static final Logger s_log = Logger.getLogger(Vocabulary.class);
	...
	public Vocabulary(String vocabularyName, String vocabularyFormat, String documentLanguage) {
		...
		if (vocabularyFormat.equals("skos")) {
			...
			if (!SKOS.exists()){
				s_log.error("File VOCABULARIES/" + vocabularyName + ".rdf does not exist.");
				System.exit(1);
			} 
			useSkos = true;
		...
		//	RT = new File("VOCABULARIES/" + vocabularyName + ".pairs.p1");
			...
			if (!EN.exists()) {
				s_log.error("File VOCABULARIES/" + vocabularyName + ".en does not exist.");
				System.exit(1);
			}		
			if (!USE.exists()) {
				s_log.error("File VOCABULARIES/" + vocabularyName + ".list.use does not exist.");
				System.exit(1);
			}
			if (!REL.exists()) {
				s_log.error("File VOCABULARIES/" + vocabularyName + ".rel.p1 does not exist.");
				System.exit(1);
			}
//			if (!RT.exists()) {
//				s_log.error("File VOCABULARIES/" + vocabularyName + ".pairs.p1 does not exist.");
//				System.exit(1);
//			}
	...
	public void buildSKOS() throws Exception {
		...
		s_log.debug("-- Building the Vocabulary index from SKOS file");
    	    ...
    	    while (iter.hasNext()) {
    	    	...
    	    	if (rel.equals("prefLabel")) {
    	    		...
    	    		if (val.contains("@")) {
    	    			...
    	    			// s_log.warn(val_components[1] + " " + m_language);
	...
	public void build() throws Exception {
		...
		s_log.debug("-- Building the Vocabulary index");
...
//					probably just related terms (cf. latest edition of Agrovoc)
	...
	public void buildREL() throws Exception {
		if (!useSkos) {
			...
			s_log.warn("-- Building the Vocabulary index with related pairs");
...
//			}
//		} catch (Exception e) {
//			s_log.warn("You need to put the .pairs file into KEA directory"); 	
//		}
//		
//	}
//	
		...
		if (all_related != null) {
			...
			for (int d = 0; d < all_related.size(); d++) {
				...
				if (rel != null) { 
					...
					s_log.warn("Problem with " + getOrig(id) + " and " + getOrig(rel_id));	
				}
			}
    	}
    	return related;
	...
	public String pseudoPhrase(String str) {
		...
		// s_log.warn(stemmed + "\t" + str_nostop + "\t"+ str);
		pseudophrase = sort(stemmed.split(" "));
		//s_log.warn(join(pseudophrase));
