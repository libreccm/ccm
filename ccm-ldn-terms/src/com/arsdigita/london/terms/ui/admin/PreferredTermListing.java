...
/**
 * Displays the preferred terms for the current term.
 * 
 * @author <a href="https://sourceforge.net/users/terry_permeance/">terry_permeance</a>
 */
public class PreferredTermListing extends AbstractDomainObjectList {
    ...
    public PreferredTermListing(DomainObjectParameter domain,
                              DomainObjectParameter term) {
        super("preferredTermListing",
              Terms.XML_PREFIX,
              Terms.XML_NS);
        ...
        addDomainObjectActionListener(
            ...
            new DomainObjectActionListener() {
                public void actionPerformed(DomainObjectActionEvent e) {
                    ...
                    term.removePreferredTerm(otherTerm);
                }
            });
    }

    /**
     * Only generate the terms list for a non-preferred term.
     * @see Term#isNonPreferredTerm()
     */
    public void generateXML(PageState state, Element parent) {
        Term term = (Term)state.getValue(m_term);       
        if (term.isNonPreferredTerm()) {
    ...
    protected DomainCollection getDomainObjects(PageState state) {
        Term term = (Term)state.getValue(m_term);
        DomainCollection terms = term.getPreferredTerms();
