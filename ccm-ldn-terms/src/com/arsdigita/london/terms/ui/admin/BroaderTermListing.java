...
/**
 * Displays the broader terms for the current term.
 * 
 * @author <a href="https://sourceforge.net/users/terry_permeance/">terry_permeance</a>
 */
public class BroaderTermListing extends AbstractDomainObjectList {
    ...
    public BroaderTermListing(DomainObjectParameter domain, DomainObjectParameter term) {
        super("broaderTermListing", Terms.XML_PREFIX, Terms.XML_NS);
        ...
        addDomainObjectActionListener(ACTION_VIEW, new DomainObjectActionListener() {
            public void actionPerformed(DomainObjectActionEvent e) {
                e.getPageState().setValue(m_term, e.getObject());
            }
        });
        addDomainObjectActionListener(ACTION_REMOVE, new DomainObjectActionListener() {
            public void actionPerformed(DomainObjectActionEvent e) {
                PageState state = e.getPageState();
                Term term = (Term) state.getValue(m_term);
                Term otherTerm = (Term) e.getObject();
                otherTerm.removeNarrowerTerm(term);
            }
        });
    }
    ...
    protected Element generateObjectXML(PageState state, DomainObject dobj) {
        Term term = (Term) state.getValue(m_term);
    ...
    protected DomainCollection getDomainObjects(PageState state) {
        Term term = (Term) state.getValue(m_term);
        DomainCollection terms = term.getBroaderTerms();
