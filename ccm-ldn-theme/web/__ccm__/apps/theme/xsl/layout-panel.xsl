...
<!-- TODO: this is a copy of the file at /cms/web/__ccm__/static/cms/admin/layout-panel/layout-panel.xsl and should be deleted when that file is moved in to core. -->
        ...
        <tr>
          <xsl:choose>
            <xsl:when test="bebop:left and bebop:body and bebop:right">
              <td style="width: 20%" class="left" valign="top"><xsl:apply-templates select="bebop:left/*"/></td>
              <td style="width: 60%" class="body" valign="top"><xsl:apply-templates select="bebop:body/*"/></td>
