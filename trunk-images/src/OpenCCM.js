...
OpenCCM.findButton = function(string) {
...
OpenCCM._lc = function(string) {
  return Xinha._lc(string, "OpenCCM");
};

OpenCCM.onGenerateOnce = function() {
  this.loadAssets();
};

OpenCCM.loadAssets = function() {
  var self = OpenCCM;
  ...
  self.loading = true;
  // Image
  Xinha._getback(Xinha.getPluginDir("OpenCCM") + "/images/images.js", function(
    getback
  ) {
    eval(getback);
    self.imagesMethodsReady = true;
  });
  Xinha._getback(
    Xinha.getPluginDir("OpenCCM") + "/images/images.html",
    function(getback) {
      OpenCCM.imagesHtml = getback;
      self.imagesDialogReady = true;
    }
  );
};

OpenCCM.showImageDialog = function(image) {
  ...
  if (image && image.tagName.toLowerCase() == "img") {
    ...
    if (image.parentNode.tagName.toLowerCase() == "div") {
      ...
      data.caption =
        image.nextSibling.tagName.toLowerCase() == "span" &&
        image.nextSibling.firstChild
          ? image.nextSibling.firstChild.nodeValue
          : "";
      ...
      data.caption =
        image.parentNode.nextSibling.tagName.toLowerCase() == "span" &&
        image.parentNode.nextSibling.firstChild
          ? image.parentNode.nextSibling.firstChild.nodeValue
          : "";
    }
  ...
  if (data.src != "") {
    this.dialogs["images"].getElementById(
      this.dialogs["images"].id["ipreview"]
    ).src = data.src;
    this.resizePreview(this.dialogs["images"]);
    this.dialogs["images"].getElementById(
      this.dialogs["images"].id["preview"]
    ).style.display = "block";
  } else {
...
OpenCCM.prepareImageDialog = function() {
  var self = this;
  var editor = this.editor;

  var dialog = (this.dialogs["images"] = new Xinha.Dialog(
    editor,
    OpenCCM.imagesHtml,
    "Xinha",
    { width: 410 }
...
OpenCCM.imageBrowse = function(window) {
...
OpenCCM.imageSet = function(imageData) {
  ...
  dialog.getElementById(dialog.id["aspect"]).value =
...
OpenCCM._getCombinedComponentHeight = function() {
  var dialog = this.dialogs["images"];
  return (
    dialog.getElementById(dialog.id["h1"]).offsetHeight +
...
OpenCCM.resizeDialog = function(dialog) {
  if (dialog.getElementById(dialog.id["preview"]).style.display == "block") {
    // Recalculate height of preview
    dialog.getElementById(dialog.id["preview"]).style.height =
      Math.min(
        Math.max(0, dialog.height - this._getCombinedComponentHeight() - 20),
        Math.round(
          window.innerHeight * 0.8 - this._getCombinedComponentHeight()
        )
      ) + "px";
    // Resize preview image
    this.resizePreview(dialog);
  }
  dialog.width = Math.min(
    Math.max(10, dialog.width),
    Math.round(window.innerWidth * 0.8)
  );
  dialog.height = Math.min(
    Math.max(10, dialog.height),
    Math.round(window.innerHeight * 0.8)
  );
};

OpenCCM.resizePreview = function(dialog) {
  ...
  var height = parseInt(
    dialog.getElementById(dialog.id["ipreview"]).height,
    10
  );
  ...
  var w = (dialog.getElementById(dialog.id["ipreview"]).style.width =
    Math.round(width * zoom) + "px");
  var h = (dialog.getElementById(dialog.id["ipreview"]).style.height =
    Math.round(height * zoom) + "px");
};

OpenCCM.calcWidth = function() {
  var dialog = this.dialogs["images"];
  dialog.getElementById(dialog.id["width"]).value = Math.round(
    dialog.getElementById("height").value *
      dialog.getElementById("aspect").value
  );
};

OpenCCM.calcHeight = function() {
  var dialog = this.dialogs["images"];
  dialog.getElementById(dialog.id["height"]).value = Math.round(
    dialog.getElementById("width").value / dialog.getElementById("aspect").value
  );
};
