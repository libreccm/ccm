...
/**
 * This class represents a drop-in FormSection for color selection. The class
 * includes a title label describing what the color refers to, a radiogroup for
 * the color choices, and a text input field for use when the available pallette
 * does not include a desired color. Note: All color values are represented
 * within the class by strings of the type <code>"#FFCA04"</code>. Neither
 * the input on the text field, or the addColor method are checked for proper
 * format - care must be taken by the user to maintain the proper string
 * representation.
 * 
 * <p>
 * The ColorPicker class includes a built in default group of 12 colors. If the
 * developer would rather supply their own colors, <code>clearColors()</code>
 * can be called, and then the internal radioGroup populated with colors
 * provided by a radioGroup using the <code>addOptionGroup()</code> method.
 * 
 * <p>
 * The <code>getValue()</code> method for this class checks first if the text
 * field is null. If not, its value is returned. If null, the radiogroup is then
 * checked for its value and returned.
 * 
 * @author Jim Parsons
 */
