...
>
    ...
    <xsl:template name="CT_SciDepartment_graphics"
        ...
        <xsl:variable name="showRole">
            <xsl:call-template name="mandalay:getSetting">
                <xsl:with-param name="module" select="'SciDepartment'"/>
                <xsl:with-param name="setting" select="concat('membersTab/', $selectedStatus, '/showRole')"/>
                <xsl:with-param name="default" select="'false'"/>
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="showStatus">
            <xsl:call-template name="mandalay:getSetting">
                <xsl:with-param name="module" select="'SciDepartment'"/>
                <xsl:with-param name="setting" select="concat('membersTab/', $selectedStatus, '/showStatus')"/>
                <xsl:with-param name="default" select="'false'"/>
            </xsl:call-template>
        </xsl:variable>
            ...
            <ul class="memberList departmentMemberList">
                <xsl:for-each select="./member">
                    <li class="sciMember">
                        ...
                        <xsl:if test="$showRole = 'true'">
                            <xsl:variable name="rolePrefix">
                                <xsl:call-template name="mandalay:getStaticText">
                                    <xsl:with-param name="module" select="'SciDepartment'"/>
                                    <xsl:with-param name="id" select="'membersTab/rolePrefix'"/>
                                </xsl:call-template>
                            </xsl:variable>
                            <xsl:variable name="rolePostfix">
                                <xsl:call-template name="mandalay:getStaticText">
                                    <xsl:with-param name="module" select="'SciDepartment'"/>
                                    <xsl:with-param name="id" select="'membersTab/rolePostfix'"/>
                                </xsl:call-template>
                            </xsl:variable>
                            <span class="role">
                                ...
                                <xsl:if test="string-length($rolePrefix) &gt; 0">
                                    <span>
                                        <xsl:value-of select="$rolePrefix"/>
                                    </span>
                                </xsl:if>
                                <xsl:value-of select="../roles/role[@key=$role]"/>
                                <xsl:if test="string-length($rolePostfix) &gt; 0">
                                    <span>
                                        <xsl:value-of select="$rolePostfix"/>
                                    </span>
                                </xsl:if>
                            </span>
                        </xsl:if>
                        <xsl:if test="$showStatus = 'true'">
                            <xsl:variable name="statusPrefix">
                                <xsl:call-template name="mandalay:getStaticText">
                                    <xsl:with-param name="module" select="'SciDepartment'"/>
                                    <xsl:with-param name="id" select="'membersTab/statusPrefix'"/>
                                </xsl:call-template>
                            </xsl:variable>
                            <xsl:variable name="statusPostfix">
                                <xsl:call-template name="mandalay:getStaticText">
                                    <xsl:with-param name="module" select="'SciDepartment'"/>
                                    <xsl:with-param name="id" select="'membersTab/statusPostfix'"/>
                                </xsl:call-template>
                            </xsl:variable>
                            <span class="status">
                                ...
                                <xsl:if test="string-length($statusPrefix) &gt; 0">
                                    <span>
                                        <xsl:value-of select="$status"/>
                                    </span>
                                </xsl:if>
                                <xsl:value-of select="../statusValues/status[@key=$status]"/>
                                <xsl:if test="string-length($statusPostfix) &gt; 0">
                                    <span>
                                        <xsl:value-of select="$statusPostfix"/>
                                    </span>
                                </xsl:if>
                            </span>
                        </xsl:if>
