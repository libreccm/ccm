...
<!-- DE
  Hier wird das Portlet Bookmark verarbeitet 
-->

<!-- EN
  Processing portlet bookmark
-->
...
<xsl:stylesheet 
  ...
  <xsl:template match="portlet:bookmark" mode="setHeading">
    <xsl:variable name="setHeading">
      <xsl:call-template name="mandalay:getSetting">
        <xsl:with-param name="module" select="'portletBookmark'"/>
      ...
      <xsl:for-each select="@applicationlink">
        <a>
          ...
          <xsl:call-template name="mandalay:getStaticText">
            ...
            <xsl:with-param name="id" select="'bookmark/edit/link'"/>
          </xsl:call-template>
        </a>
        <br/>
      </xsl:for-each>
