...
public class ThemeDirectorConfig extends AbstractConfig {
     ...
     * set com.arsdigita.themedirector.ThemeDirectorConfig=DEBUG by uncommenting
     * or adding the line.
     */
    ...
    /**
     * Number of seconds between checks for updated development files in a
     * multi-jvm installation.
     */
    private final Parameter m_themeDevFileWatchPollDelay = new IntegerParameter(
        "themedirector.theme_dev_file_watch_poll_delay",
        Parameter.REQUIRED, new Integer(60 * 90));   //once every 90 minutes

    /**
     * Number of seconds before checking for update of published theme files in
    ...
    /**
     * number of seconds between checks for recently published theme files in a
     * multi-jvm installation.
     */
    private final Parameter m_themePubFileWatchPollDelay = new IntegerParameter(
     ...
     * the constructor directly! Use getInstance() instead.
     */
     ...
     * files to copy into a new theme as a default implementation. By default it
     * is set to a master directory containing the distribution's default theme.
     * When creating a new theme it is copied over to provide a default for the
     * new theme.
     *
     * Developer's note: Previously it was used as a string to filter a files
     * directory stored in the Manifest file. Matching files were copied to the
     * new theme's directory. The reason to use this approach is not documented.
     * As a guess: it enables to copy from more than one directory, if those
     * directories share a common name part which is specified here. But because
     * all files must be present at deploy time to be included into the Manifest
     * file it makes no sense. From original comment: "Specifically, if this is
     * not null (or the empty string) than any file that is used as part of the
     * default directory must start with this string."
     *
    ...
    public static String getDefaultThemeURL(HttpServletRequest req) {
        ...
        if (themeApps.next()) {
            ...
            if (s_log.isDebugEnabled()) {
                ...
                s_log.debug("Default Theme URL: " + themeApps.get(
                    DEFAULT_THEME_URL));
            }
    ...
    /**
     * The number of seconds to wait before checking the database for the first
     * time. A value of 0 means that the thread should not be started. This
     * checks for published files.
     *
     * @return
     */
    ...
    /**
     * Returns the number of seconds between checking for updated files in the
     * file system. This checks for published files.
     *
     * @return
     */
    ...
    /**
     * Returns the number of seconds between checking for updated files in the
     * file system. This checks for development files.
     *
     * @return
     */
