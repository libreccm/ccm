...
/**
 * Class for polling the database to look for new/updated published files in the
 * ThemeFile table.
 *
 * For "published" files, It goes through each Theme and looks at the last time
 * it was published. If the last time published &gt; last time thread was run
 * then it examines those files. When the thread runs for the first time, it
 * examines all themes.
 *
...
public class ThemePublishedFileManager extends ThemeFileManager {
     ...
     * set com.arsdigita.themedirector.util.ThemePublishedFileManager=DEBUG by
     ...
     * Initializer() provides null as baseDirectory parameter because it doesn't
     * know about servlet context and can not determine the directory.
     *
     ...
     *                      be null! (Most likely if invokel by Initializer!)
     */
    ...
    public static ThemeFileManager startWatchingFiles(int startupDelay,
        ...
        if (s_manager == null) {
            ...
            if (startupDelay > 0) {
                ...
                s_log.warn(
                    "Theme Published File Manager Thread is not starting "
    ...
    /**
     * This returns the current thread or null if the thread has not yet been
     * started.
     *
     * @return
     */
    ...
    protected void updateTheme(Theme theme) {
        if (getLastRunDate() == null
                || m_ignoreInterrupt
                || !(new File(getManagerSpecificDirectory()
                              + theme.getURL())).exists()
