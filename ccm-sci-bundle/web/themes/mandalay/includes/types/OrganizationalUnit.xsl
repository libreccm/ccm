...
<xsl:stylesheet 
  ...
  <xsl:template name="CT_OrgnizationalUnit_graphics" match="cms:item[objectType='com.arsdigita.cms.contenttypes.OrganizationalUnit']" mode="detailed_view">
    ...
    <xsl:variable name="directionSetLink">
      <xsl:call-template name="mandalay:getSetting">
        <xsl:with-param name="module" select="'OrganizationalUnit'"/>
        <xsl:with-param name="setting" select="'directionSetLink'"/>
        <xsl:with-param name="default" select="'true'"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="setAssistentDirection">
      <xsl:call-template name="mandalay:getSetting">
        <xsl:with-param name="module" select="'OrganizationalUnit'"/>
        <xsl:with-param name="setting" select="'setAssistentDirection'"/>
        <xsl:with-param name="default" select="'true'"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="assistentDirectionSetLink">
      <xsl:call-template name="mandalay:getSetting">
        <xsl:with-param name="module" select="'OrganizationalUnit'"/>
        <xsl:with-param name="setting" select="'assistentDirectionSetLink'"/>
        <xsl:with-param name="default" select="'true'"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="setSubUnits">
      <xsl:call-template name="mandalay:getSetting">
        <xsl:with-param name="module" select="'OrganizationalUnit'"/>
        <xsl:with-param name="setting" select="'setSubUnits'"/>
        <xsl:with-param name="default" select="'true'"/>
      </xsl:call-template>      
    </xsl:variable>
    <xsl:variable name="subUnitsSetLink">
      <xsl:call-template name="mandalay:getSetting">
        <xsl:with-param name="module" select="'OrganizationalUnit'"/>
        <xsl:with-param name="setting" select="'subUnitsSetLink'"/>
        <xsl:with-param name="default" select="'true'"/>
      </xsl:call-template>      
    </xsl:variable>
    ...
    <div id="details">
      ...
      <xsl:if test="$setDirection = 'true'">
        <div id="organizationalUnitDirection">
          <h2>
          ...
          <xsl:choose>           
            <xsl:when test="$directionSetLink = 'true'">
              <a>
      ...
      <xsl:if test="$setAssistentDirection = 'true'">
        <div id="organizationalUnitAssistentDirection">
          <h2>
            <xsl:call-template name="mandalay:getStaticText">
              <xsl:with-param name="module" select="'OrganizationalUnit'"/>
              <xsl:with-param name="id" select="'assistentDirectionTitle'"/>
            </xsl:call-template>
          </h2>
          <xsl:choose>
            <xsl:when test="$assistentDirectionSetLink">
              <a>
                <xsl:attribute name="href">
                  <xsl:text>/redirect?oid=</xsl:text>
    ...
    <div id="organizationalUnitSubUnits">
      <h2>
        <xsl:call-template name="mandalay:getStaticText">
          <xsl:with-param name="module" select="'OrganizationalUnit'"/>
          <xsl:with-param name="id" select="'subunitsTitle'"/>
        </xsl:call-template>        
      </h2>
      <ul>
        <xsl:for-each select="subunits">
          <li>
            <xsl:choose>
              <xsl:when test="$subUnitsSetLink = 'true'">
                <a>
                  <xsl:attribute name="href">
                    <xsl:text>/redirect?oid=</xsl:text>
    ...
    <div id="organizationalUnitMembers">
      <h2>
        <xsl:call-template name="mandalay:getStaticText">
          <xsl:with-param name="module" select="'OrganizationalUnit'"/>
  ...
  <!-- DE Listenansicht -->
  <!-- EN List view -->
  <xsl:template name="CT_OrganizationalUnit_List" match="nav:item[nav:attribute[@name='objectType'] = 'com.arsdigita.cms.contenttypes.OrganizationalUnit']" mode="list_view">
    <!-- DE Hole alle benötigten Einstellungen-->
    <!-- EN Getting all needed setting-->
  ...
  <xsl:template name="CT_OrganizationalUnit_Link" match="*/cms:item/links[targetItem/objectType = 'com.arsdigita.cms.contenttypes.OrganizationalUnit']" mode="link_view">
