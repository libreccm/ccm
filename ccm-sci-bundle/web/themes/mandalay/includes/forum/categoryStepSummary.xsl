...
<!-- DE
  Hier werden die {Name} verarbeitet 
-->
...
<xsl:stylesheet 
  ...
  <xsl:template match="forum:categoryRoots">
  ...
  <xsl:template match="forum:categoryRoot">
    ...
    <dd>
      ...
      <xsl:choose>
        <xsl:when test="count(../../forum:itemCategories/forum:itemCategory[starts-with(@path, $catName)]) = 0">
          <div>
            <xsl:call-template name="mandalay:getStaticText">
              <xsl:with-param name="module" select="'cms'"/>
        ...
        <xsl:otherwise>
          <ul>
            <xsl:apply-templates select="../../forum:itemCategories/forum:itemCategory[starts-with(@path, $catName)]" mode="list">
  ...
  <!-- DE Zeige eine zugewiesene Kategorie an und biete die Möglichkeit, diese wieder zu entfernen. Da
    dieses Template nur aus dem forum:categoryRoot heraus aufgerufen werden soll, wird hier der 
  ...
  <!-- EN Show an assigned category and a link to remove this assignment. Because this template is meant
    to be called only by forum:categoryRoot, there is a mode="list" added. Otherwise there will be
    a duplicated list below the list of category roots. -->
  <xsl:template match="forum:itemCategory" mode="list">
  ...
  <xsl:template match="forum:categoryWidget">
    ...
    <xsl:choose>
      <xsl:when test="@mode='javascript'">
        <ul>
          <xsl:apply-templates select="forum:category" mode="javascript"/>
        </ul>
        <xsl:apply-templates select="forum:selectedCategories"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <!-- DE -->
  <!-- EN -->
  <xsl:template match="forum:selectedCategories">
    <select id="catWdHd" name="{../@name}" size="5" multiple="multiple" style="display: none">
      <xsl:apply-templates select="forum:category" mode="hidden"/>
    </select>
  </xsl:template>
  
  <!-- DE -->
  <!-- EN -->
  <xsl:template match="forum:category" mode="hidden">
    <option value="{@id}">
  ...
  <xsl:template match="forum:category" mode="javascript">
    
    <xsl:variable name="onlyOneLevel">
      <xsl:choose>
        <xsl:when test="not(@root = '1' or forum:category/@expand = 'all') and not(//forum:categoryWidget)">
    ...
    <li id="catSelf{@id}">
      ...
      <xsl:variable name="treeToggleMode">
        <xsl:choose>
          <xsl:when test="@root = '1' or forum:category/@expand = 'all'">
      ...
      <xsl:variable name="selCats">
        <xsl:for-each select="//forum:categoryWidget/forum:selectedCategories/forum:category">
          <xsl:choose>
      ...
      <xsl:choose>
        <xsl:when test="forum:category and $onlyOneLevel = 'no'">
              ...
              <xsl:attribute name="src">
                <xsl:call-template name="mandalay:linkParser">
                  <xsl:with-param name="link">
                    <xsl:call-template name="mandalay:getSetting">
                      <xsl:with-param name="module" select="'cms'"/>
                      <xsl:with-param name="setting" select="'setImages/categoryCollapse'"/>
        ...
        <xsl:when test="(@root = '1' and not(forum:category)) or ($onlyOneLevel = 'yes' and forum:category)">
          <a id="catTreeToggleLink{@node-id}" href="#" onclick="{$treeToggleMode}('{@node-id}', '{$selCats}');">
            <img id="catTreeToggleImage{@node-id}" alt="[+]">
              <xsl:attribute name="src">
                <xsl:call-template name="mandalay:linkParser">
  ...
  <xsl:template match="forum:category" mode="plain">
    <option value="{@id}">
  ...
  <!-- DE -->
  <!-- EN -->
  <xsl:template match="forum:emptyPage[@title='childCategories']">
    <xsl:choose>
      <xsl:when test="forum:category/@order='sortKey'">
        <xsl:apply-templates select="forum:category/forum:category" mode="javascript">
          <xsl:sort data-type="number" select="@sortKey"/>
        </xsl:apply-templates>
      </xsl:when>
      <xsl:otherwise>
  ...
  <!-- DE -->
  <!-- EN -->
  <xsl:template match="forum:emptyPage[@title='autoCategories']">
    <xsl:choose>
      <xsl:when test="forum:category/@order='sortKey'">
        <xsl:apply-templates select="forum:category" mode="javascript" >
          <xsl:sort data-type="number" select="@sortKey"/>
